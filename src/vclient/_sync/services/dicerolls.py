...
class SyncDicerollService(SyncBaseService):
    ...
    def get_page(self, *, limit: int=DEFAULT_PAGE_LIMIT, offset: int=0, userid: str | None=None, characterid: str | None=None, campaignid: str | None=None) -> PaginatedResponse[Diceroll]:
        """Retrieve a paginated page of dice rolls."""
        return self._get_paginated_as(self._format_endpoint(Endpoints.DICEROLLS), Diceroll, limit=limit, offset=offset, params=self._build_params(userid=userid, characterid=characterid, campaignid=campaignid))

    def list_all(self, *, userid: str | None=None, characterid: str | None=None, campaignid: str | None=None) -> list[Diceroll]:
        """Retrieve all dice rolls."""
        return [diceroll for diceroll in self.iter_all(userid=userid, characterid=characterid, campaignid=campaignid)]

    def iter_all(self, *, userid: str | None=None, characterid: str | None=None, campaignid: str | None=None, limit: int=100) -> Iterator[Diceroll]:
        """Iterate through all dice rolls."""
        for item in self._iter_all_pages(self._format_endpoint(Endpoints.DICEROLLS), limit=limit, params=self._build_params(userid=userid, characterid=characterid, campaignid=campaignid)):
    ...
    def create(self, request: DicerollCreate | None=None, **kwargs) -> Diceroll:
        ...
        response = self._post(self._format_endpoint(Endpoints.DICEROLLS), json=body.model_dump(exclude_none=True, exclude_unset=True, mode="json"))
        return Diceroll.model_validate(response.json())

    def create_from_quickroll(self, *, quickroll_id: str, character_id: str, comment: str | None=None, difficulty: int=6, num_desperation_dice: int=0) -> Diceroll:
        """Create a new dice roll using a quickroll template."""
        response = self._post(self._format_endpoint(Endpoints.DICEROLL_QUICKROLL), json=_DicerollQuickrollCreate(quickroll_id=quickroll_id, character_id=character_id, comment=comment, difficulty=difficulty, num_desperation_dice=num_desperation_dice).model_dump(exclude_none=True, exclude_unset=True, mode="json"))
        return Diceroll.model_validate(response.json())
