...
class SyncBaseService:
    ...
    def _put_file(self, path: str, *, file: tuple[str, bytes, str]) -> httpx.Response:
        """Make a PUT request with a file upload (multipart/form-data).

        PUT is idempotent, so this helper takes no idempotency key.
        ...
        """
        filename, content, content_type = file
        return self._request("PUT", path, files={"data": (filename, content, content_type)})
