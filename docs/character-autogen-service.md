...
Auto-generate characters for a campaign using AI-assisted character creation.
...
# Or use interactive session for multiple choices
session = await autogen.start_chargen_session()
print(f"Session expires: {session.expires_at}")
print(f"Characters to choose from: {len(session.characters)}")

# Select one of the generated characters
...
See [Response Models](models.md) for `Character` and `ChargenSessionResponse`.
